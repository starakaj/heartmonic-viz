...
function onDocumentMouseUp( event ) {
	...
	for (let i=0; i<leftLineGeometry.vertices.length; i++) {
		...
		// Now, add the faces
		// if (i>0) {
		// 	let faces = makeStripFaces((i-1)*verticesPerStrip, i*verticesPerStrip, verticesPerStrip);
		// 	for (let j=0; j<faces.length; j++) {
		// 		stripGeometry.faces.push(faces[j]);
		// 		for (let k=0; k<3; k++) {
		// 			let vidx = faces[j][faceIndices[ k ]];
		// 			let vdist = Math.floor(vidx / brushSamples); // Which sample band are we in
		// 			let vheight = heightForVertexAtIndex( vidx );
		// 			let vcolor = colorForVertexAtIndex( vidx );
		// 			faces[j].vertexColors[ k ] = vcolor;
		// 		}
		// 	}
		// }
	}
	...
	// let brushMaterial = new THREE.MeshBasicMaterial( { color: 0xff0000 } );
